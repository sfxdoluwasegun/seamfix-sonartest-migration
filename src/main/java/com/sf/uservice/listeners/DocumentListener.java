 ...
 * Sends documents queued for syncing for operation
 * 
 * @author KOMOO
 *
 */

@MessageDriven(mappedName = "queue/DocumentSync", activationConfig = {
		@ActivationConfigProperty(propertyName = "destinationType", propertyValue = "javax.jms.Queue"), 
		@ActivationConfigProperty(propertyName="destination", propertyValue="queue/DocumentSync"), 
...
public class DocumentListener implements MessageListener {
	...
	public void onMessage(Message message) {
		...
		try{
			...
			syncOps.synchronizeAlumniDocument(document);
		}
		catch(JMSException e){
			log.error("DocumentListener.onMessage", e);
		}
